...
namespace Canister.Tests
{
    ...
    public class ComponentFactoryViewTests
    {
        ...
        public void RegisterSingleComponent()
        {
            ...
            // assert
            cache.GetComponentFactory(event1.OriginalComponentKey).Should().Be(event1.ComponentFactory);
        }
        ...
        public void RegisterSingleComponentWithAssignedTypes()
        {
            ...
            cache.GetComponentFactory(event1.OriginalComponentKey).Should().Be(null);
        }
        ...
        public void RegisterMultipleComponentsWithAssignedTypes()
        {
            ...
            cache.GetComponentFactory(event1.OriginalComponentKey).Should().Be(null);
            cache.GetComponentFactory(event3.OriginalComponentKey).Should().Be(null);
        }
        ...
        public void RegisterMultipleComponentsWithAssignedKeysAndPreservedRegistrations2()
        {
            ...
            var event4 = new ComponentKeysAssigned(event3.ComponentRegistrationId, new[] { typeof(IDoStuff) });
            var event5 = new ComponentRegistered(Guid.NewGuid(), typeof(DoEverything), resolver => new DoEverything());
            var event6 = new ComponentKeysAssigned(event3.ComponentRegistrationId, new[] { typeof(IDoStuff), typeof(IDoOtherStuff) });
            var event7 = new ExistingRegistrationsPreserved(event3.ComponentRegistrationId);
            ...
            // assert
            cache.GetComponentFactory(event4.ComponentKeys[0]).Should().Be(event5.ComponentFactory);
            cache.GetComponentFactory(event1.OriginalComponentKey).Should().Be(null);
            cache.GetComponentFactory(event3.OriginalComponentKey).Should().Be(null);
            cache.GetComponentFactory(event5.OriginalComponentKey).Should().Be(null);
        }
    }
}
